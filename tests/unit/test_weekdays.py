...
import pytest
from dou.domain.service.weekdays import (
    findpascoa,
    gethollidays,
    get_weekdays,
...
class TestGetHollidays:
    ...
    def test_gethollidays_contains_fixed_holidays(self):
        ...
        fixed_holidays = [(1, 1), (21, 4), (1, 5), (7, 9), (12, 10), (2, 11), (15, 11), (25, 12)]
...
class TestGetWeekdays:
    ...
    def test_get_weekdays_excludes_holidays(self):
        ...
        assert not any(day.day == 7 and day.month == 4 for day in weekdays)  # Good Friday
        assert not any(day.day == 21 and day.month == 4 for day in weekdays)  # Tiradentes
...
class TestWeekdaysIntegration:
    ...
    def test_holiday_calculation_consistency(self):
        ...
        for year in [2020, 2021, 2022, 2023, 2024, 2025]:
            ...
            fixed_holidays = [(1, 1), (21, 4), (1, 5), (7, 9), (12, 10), (2, 11), (15, 11), (25, 12)]
            for holiday in fixed_holidays:
                assert holiday in holidays, f"Fixed holiday {holiday} missing for year {year}"
            
            # Should contain Easter-related holidays (dates vary by year)
            easter_related = [h for h in holidays if h not in fixed_holidays]
            assert len(easter_related) >= 3, f"Missing Easter-related holidays for year {year}"  # Good Friday + 2 Carnival days