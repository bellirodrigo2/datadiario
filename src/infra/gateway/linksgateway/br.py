...
from src.infra.web.webscrapper import WebScrapper


async def get_br_dou1_links(date: datetime) -> list[str]:
    return _get_br_links("dou1", date)


async def get_br_dou2_links(date: datetime) -> list[str]:
    return _get_br_links("dou2", date)


async def get_br_dou3_links(date: datetime) -> list[str]:
