...
class LinkReader(UseCase):
    ...
    async def execute(
        ...
        try:
            links = await self.links_repo.get_links(entity_name, group, date)

            if status_filter:
                # Filter by status if specified
                filtered_links = [
                    link for link in links if link.status == status_filter
                ]
                self.logger.info(
                    f"Retrieved {len(filtered_links)} {status_filter.value} links for {entity_name}:{group} on {date}"
                )
                return filtered_links

            self.logger.info(
                f"Retrieved {len(links)} links for {entity_name}:{group} on {date}"
            )
            return links

        except Exception as e:
            self.logger.error(
                f"Error reading links for {entity_name}:{group} on {date}: {e}"
            )
            raise
...
class LinkReaderRange(UseCase):
    ...
    async def execute(
        ...
        from dou.domain.service.weekdays import get_weekdays_from_range
        ...
        for weekday in weekdays:
            try:
                links = await self.link_reader.execute(entity_name, group, weekday, status_filter)
